...
function tabulate(datatable, columns, containerString) {
    ...
    /*
    // Replace [audio:xxx.mp3] with <audio> elements
    // Do this without d3.js, since it is not needed
    var audioRegex = /\[sound:([^\]\s]+\.mp3)\]/ig;
        ...
        for (var key in row) {
            if (row.hasOwnProperty(key)) {
                ...
                if (typeof value === 'string') {
                    // Replace [audio:xxx.mp3] with <audio> elements
                    value = value.replace(audioRegex, function(match, filename) {
    ...
    datatable.forEach(function(row) {
        for (var key in row) {
            if (row.hasOwnProperty(key)) {
                ...
                if (typeof value === 'string') {
                    // Remove [audio:xxx.mp3]
                    value = value.replace(audioRegex, '');
                    row[key] = value;
                }
            }
        }
...
function sqlToTable(uInt8ArraySQLdb) {
    ...
    // Lingosia - Start
    // Show the number of items in the largest array of deckNotes
    if (deckNotes && deckNotes.length > 0) {
        var maxNotes = Math.max.apply(null, deckNotes.map(function(deck) {
            return deck.notes.length;
        }));
        console.debug("Largest deckNotes.notes array length:", maxNotes);
    } else {
        console.debug("deckNotes is empty or undefined");
    }

    // Remove sound tags from deckNotes
    var soundRegex = /\[sound:([^\]\s]+\.mp3)\]/ig;
    deckNotes.forEach(function(deck) {
        deck.notes.forEach(function(note) {
            for (var key in note) {
                if (note.hasOwnProperty(key)) {
                    var value = note[key];
                    if (typeof value === 'string') {
                        // Remove [sound:xxx.mp3]
                        value = value.replace(soundRegex, '');
                        note[key] = value;
                    }
                }
            }
    ...
    // Remove img tags from deckNotes
    var imgRegex = /<img[^>]*>/ig;
    deckNotes.forEach(function(deck) {
        deck.notes.forEach(function(note) {
            for (var key in note) {
    ...
    // Remove html tags from deckNotes
    var htmlRegex = /<\/?[^>]+(>|$)/g;
    deckNotes.forEach(function(deck) {
        deck.notes.forEach(function(note) {
            for (var key in note) {
    ...
    // Save deckNotes in the local storage so that it can be used later

    // Get language from cookie
    var language = document.cookie.replace(/(?:(?:^|.*;\s*)language\s*\=\s*([^;]*).*$)|^.*$/, "$1");

    localStorage.setItem("ankiDeck_" + language, JSON.stringify(deckNotes));
    // Lingosia - End
...
function parseImages(imageTable,unzip,filenames){
    ...
    var numberOfImagesToProcess = d3.selectAll("img")[0].length;
    console.debug("Number of images:", numberOfImagesToProcess);
    var numberOfImagesProcessed = 0;

    d3.selectAll("img") //"img"
      .attr("src", function(d,i) {
        ...
        if (key in map){
            ...
            console.debug("Processing image " + numberOfImagesProcessed + " of " + numberOfImagesToProcess);

            //Lingosia - Start

            // Save in indexedDB so that it can be used later
            if (window.indexedDB) {
                var request = indexedDB.open("anki", 1);
    ...
    var numberOfAudioToProcess = d3.selectAll("audio")[0].length;
    console.debug("Number of audio files:", numberOfAudioToProcess);
    var numberOfAudioProcessed = 0;

    d3.selectAll("audio") //"audio"
        .attr("src", function(d,i) {
            //Some filenames may be encoded. Decode them beforehand.
            var key = decodeURI(this.firstChild.src.split('/').pop());
            if (key in map){
                numberOfAudioProcessed += 1;
                console.debug("Processing audio " + numberOfAudioProcessed + " of " + numberOfAudioToProcess);
                ...
                if (window.indexedDB) {
                    ...
                    request.onsuccess = function(event) {
                        ...
                        var store = transaction.objectStore("sound");
                        store.put(map[key], key);
                    };
                }
...
function ankiBinaryToTable(ankiArray, options) {
    ...
    // Step 1: decompress and parse SQLite
    if (filenames.indexOf(sqliteFile) >= 0) {
        ...
        if (options && options.loadImage){
          if (filenames.indexOf("media") >= 0) {
              ...
              f.readAsText(bb);
              return; // Wait for FileReader to finish before logging 100%
          }
        }
    }

    console.debug("Items imported");
    document.getElementById("ankiLoadProgress").innerHTML += "File Load complete. Your deck is imported. ✅<br>";
}
...
function readySetup() {
    ...
    var setOptionsImageLoad = function(){
        options.loadImage = false; // $('input#showImage').is(':checked'); (Lingosia edit)
        return options;
    }   
    var eventHandleToTable = function(event) {
        ...
        // Remove any prior language notes from local storage
        localStorage.removeItem("ankiDeck_" + language);

        $("#ankiLoadProgress").show();
        // Show progress message
        document.getElementById("ankiLoadProgress").innerHTML = "Loading Anki file. Please wait. 🔄<br>";
        console.debug("Loading Anki file from drive...");   
